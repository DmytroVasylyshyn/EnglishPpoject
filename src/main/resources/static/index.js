$(document).ready(function() {
    ...
    $('.card').each(function() {
        let card = $(this);
        let cancelButton = card.find('.cancel_button');
        let postButton = card.find('.post_button');
        let commentInput = card.find('.comment_input');
        let commentBlock = card.find('.comment_block');

        changeButtonBold(card.find('.bookmark i'));
        changeButtonBold(card.find('.like_button i'));
...
function addComment(commentBlock, commentAvatar, commentName, commentText) {
    ...
    `;
    commentBlock.append(commentHTML); // Додати коментар до контейнера
}

let cancelButton = $(".cancel_button");
let postButton = $(".post_button");
let commentInput = $(".comment_input");
let commentBlock = $(".comment_block");